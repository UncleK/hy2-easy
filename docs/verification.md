...
0.2.1 简化了表单，完成后固定在网页和 Agent 中提供连接信息。本机 Windows 的 6 项 Agent 测试通过，其中验证了两端返回相同的链接和二维码，SSH 登录密码不进入结果。浏览器使用演示后端检查了去掉勾选项后的填写、确认和结果流程；此次未重新测试真实服务器安装。
...
另使用 v2rayN 7.24.9 原包中 Mihomo 1.19.29 / Windows amd64 核心，验证生成配置能取得真实 HTTPS 204，替换为错误证书指纹后连接被拒绝；这不等同于所有 Clash 图形客户端已实测。
